...
{
	// import obj
	fromOBJ(operateMesh, "data/facade_extrude.obj");

	// temporary vertex colors
	vector<zColor> vertexColors;
	for (int i = 0; i < operateMesh.numVertices(); i++)
	{
		vertexColors.push_back(zColor((i * (1.0 / operateMesh.numVertices())), (i * (1.0 / operateMesh.numVertices())), (i * (1.0 / operateMesh.numVertices())), 0));
		cout << vertexColors[i].r << " " << vertexColors[i].g  << " " << vertexColors[i].b << endl;
	}
	setVertexColors(operateMesh, vertexColors, true);
...
void update(int value)
{
	if (updateMESHTOPOLOGY)
	{
		...
		if (offsetEXTRUDE)
		{
			...
			else offsetMesh = variableFaceOffset(operateMesh, keepFACES, true, min_FACEOFFSET, max_FACEOFFSET);
		}
