...
public:
	...
	void exportGCode(string fileToWrite = "C:/Users/bshajay/Desktop/test/DX100/Storage Card/A.JBI")
	{
		...
		myfile_write << "/JOB" << endl;
		myfile_write << "//NAME A" << endl;
		myfile_write << "//POS" << endl;
		myfile_write << "///NPOS " << actualPathLength - 1 << ",0,0,0,0,0" << endl;
		myfile_write << "///TOOL 0" << endl;
		myfile_write << "///POSTYPE PULSE" << endl;
		myfile_write << "///PULSE" << endl;
		...
		for (int i = 0; i < actualPathLength - 1; i++)
		{
			...
			/*double degToPulse[6] = { 1088.30918, -1213.62963, 1011.35802, -910.22222, 916.54321, -573.29888 };
			double aliceToMotoRotOffset[6] = { 0, -90, 0, 0, 0, 0 };
			double MotoRottoPulseOffset[6] = { 0, -90, 90, 0, 0, 0 };

			e_x = (rotations[i][0] + aliceToMotoRotOffset[i] + MotoRottoPulseOffset[i]) * degToPulse[0];
			e_y = (rotations[i][1] + aliceToMotoRotOffset[i] + MotoRottoPulseOffset[i]) * degToPulse[1];
			e_z = (rotations[i][2] + aliceToMotoRotOffset[i] + MotoRottoPulseOffset[i]) * degToPulse[2];
			r = (rotations[i][3] + aliceToMotoRotOffset[i] + MotoRottoPulseOffset[i]) * degToPulse[3];
			p = (rotations[i][4] + aliceToMotoRotOffset[i] + MotoRottoPulseOffset[i]) * degToPulse[4];
			y = (rotations[i][5] + aliceToMotoRotOffset[i] + MotoRottoPulseOffset[i]) * degToPulse[5];
			*/
			double degToPulse[6] = { 1088.30918, -1213.62963, 1011.35802, -910.22222, 916.54321, -573.29888 };

			e_x = rotations[i][0] * degToPulse[0];
			e_y = (rotations[i][1] - 90.0) * degToPulse[1];
			e_z = (rotations[i][2]) * degToPulse[2];
			r = rotations[i][3] * degToPulse[3];
			p = rotations[i][4] * degToPulse[4];
			y = rotations[i][5] * degToPulse[5];

			//12,16,-13 --> 12,16,-77
			sprintf_s(gcode, "C%05i=%1.2f,%1.2f,%1.2f,%1.2f,%1.2f,%1.2f", i, e_x, e_y, e_z, r, p, y);
			//C00001=-15234.2,138034,-17066.7,184115,6832.08,106464
		...
		myfile_write << "//INST" << endl;
		myfile_write << "///DATE 2016/07/03 16:48" << endl;
		myfile_write << "///ATTR SC,RW" << endl;
		myfile_write << "///GROUP1 RB1" << endl;
		myfile_write << "NOP" << endl;


		//- iterate through path
		//for (int i = actualPathLength - 2; i >= 0; i--) // reverse order
		myfile_write << "DOUT OT#(9) OFF" << endl;
		myfile_write << "DOUT OT#(9) ON" << endl;
		myfile_write << "DOUT OT#(8) 0" << endl;

		for (int i = 0; i < actualPathLength - 1; i++)
		{

			sprintf_s(gcode, "MOVJ C%05i VJ=0.5", i);
			//MOVL C00000 VJ = 10.00
